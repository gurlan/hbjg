...
<div class="container inner contanershadow">
    ...
    <div class="content right">
        ...
        <div class="com_con">
            <ul class="news_col">
                <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=039787030e0a64b02a8df26dcd4a331e&action=lists&catid=%24catid&num=12&order=listorder+ASC%2Cinputtime+desc&page=%24page\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">缂栬緫</a>";}$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'lists')) {$pagesize = 12;$page = intval($page) ? intval($page) : 1;if($page<=0){$page=1;}$offset = ($page - 1) * $pagesize;$content_total = $content_tag->count(array('catid'=>$catid,'order'=>'listorder ASC,inputtime desc','limit'=>$offset.",".$pagesize,'action'=>'lists',));$pages = pages($content_total, $page, $pagesize, $urlrule);$data = $content_tag->lists(array('catid'=>$catid,'order'=>'listorder ASC,inputtime desc','limit'=>$offset.",".$pagesize,'action'=>'lists',));}?>
                <?php $i=0?>
                <?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
                <?php if($i<2) { ?>
                <li class="first">
                    <div class="tit"><a href="<?php echo $r['url'];?>" title=" <?php echo $r['title'];?>">
                        <?php echo $r['description'];?></a><span><?php echo date("Y-m-d",$r[inputtime]);?></span></div>
                    <div class="txt">&nbsp;<?php echo $r['description'];?>
                    <a href="<?php echo $r['url'];?>">查看更多</a></div>
                </li>
                <?php } ?>
                <?php $i++?>
                <?php $n++;}unset($n); ?>
